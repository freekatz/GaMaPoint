...
from backbone import NaiveGaussian3D
from backbone.mamba_ssm.models import MambaConfig
from backbone.gama_points import PointMambaLayer
...
class DelaSemSeg(nn.Module):
    ...
    def forward(self, gs: NaiveGaussian3D):
        x = gs.gs_points.f
        xyz = gs.gs_points.p
        pts_list = gs.gs_points.pts_list
        indices = []
        for i in range(len(gs.gs_points.idx_group)):
            indices.append(gs.gs_points.idx_group[i])
            if i > 0:
                indices.append(gs.gs_points.idx_ds[i])
        for i in range(len(gs.gs_points.idx_us)):
            indices.append(gs.gs_points.idx_us[-i-1])
        indices = indices[::-1]
