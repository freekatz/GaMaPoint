...
class Stage(nn.Module):
    ...
    def forward(self, x, xyz, prev_knn, indices, pts_list):
        ...
        if not self.last:
            sub_x = self.sub_stage(x, xyz, knn, indices, pts_list)
        else:
            sub_x = None
...
class DelaSemSeg(nn.Module):
    ...
    def forward(self, gs: NaiveGaussian3D):
        ...
        for i in range(4):
            indices.append(idx_group[i])
            if i < 3:
                indices.append(idx_ds[i])
            #     print(i, 'idx_ds', idx_ds[i].shape)
            # print(i, 'idx_group', idx_group[i].shape)
            # print(i, 'idx_gs_group', idx_gs_group[i].shape)
        for i in range(3):
            indices.append(idx_us[-i-1])
            # print(i, 'idx_us', idx_us[i].shape)

        indices = indices[::-1]
        # for idx in indices:
        #     print(idx.shape)

        x = self.stage(x, xyz, None, indices, pts_list)
        return self.head(x)
